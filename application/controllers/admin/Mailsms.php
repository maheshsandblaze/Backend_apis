...
class Mailsms extends Public_Controller
{
    ...
    public function compose()
    {
        ...
        // Email templates
        $email_templates = $this->messages_model->get_email_template();
        ...
        if (!empty($birthStudents)) {
            $students = [];
            foreach ($birthStudents as $student) {
                $students[] = [
            ...
            $birthDaysList['students'] = $students;
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'data'   => [
                    'title'                   => 'Add Mailsms',
                    'classlist'               => $classlist,
                    'email_template_list'     => $email_templates,
                    'roles'                   => $this->role_model->get(),
                    'birthDaysList'           => $birthDaysList,
                    'sch_setting'             => $this->sch_setting_detail,
                    'superadmin_restriction'  => $this->customlib->superadmin_visible()
                ]
            ]));
    }
    ...
    public function compose_sms()
    {
        ...
        // SMS templates
        $sms_templates = $this->messages_model->get_sms_template();
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'data'   => [
                    'title'                  => 'Add Mailsms',
                    'classlist'              => $classlist,
                    'sms_template_list'      => $sms_templates,
                    'roles'                  => $this->role_model->get(),
                    'birthDaysList'          => $birthDaysList,
                    'sch_setting'            => $this->sch_setting_detail,
                    'send_through_list'      => $this->send_through,
                    'superadmin_restriction' => $this->customlib->superadmin_visible()
                ]
            ]));
    }
    ...
    public function send_individual()
    {
        ...
        $send_type = $input['individual_send_type'];
        $send_by   = $input['individual_send_by'];
    
        // SMS / Mail flags
        $send_sms  = ($send_by === 'sms') ? 1 : 0;
        $send_mail = ($send_by === 'sms') ? 0 : 1;
    
        // Decode users
        $userlisting = json_decode($input['user_list'], true);
        $user_array  = [];
        ...
        $data = [
            'is_individual'     => 1,
            'title'             => $input['individual_title'],
            'message'           => $input['individual_message'],
            'send_mail'         => $send_mail,
            'send_sms'          => $send_sms,
            'email_template_id' => $input['template_id'] ?? null,
            'user_list'         => json_encode($user_array),
            'created_at'        => date('Y-m-d H:i:s')
        ];
        ...
        if ($send_type !== 'send_now') {
            ...
            if (empty($input['schedule_date_time'])) {
                return $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode([
                        'status' => 1,
                        'msg'    => ['schedule_date_time' => 'Required']
                    ]));
            }
    
            $data['is_schedule']        = 1;
            $data['sent']               = 0;
            $data['schedule_date_time'] = date(
                'Y-m-d H:i:s',
                strtotime($input['schedule_date_time'])
            );
        }
        ...
        if ($send_type === 'send_now') {
            ...
            // Send Email
            if ($send_mail && !empty($this->mail_config)) {
                foreach ($user_array as $u) {
                    if (!empty($u['email'])) {
                        $this->mailer->compose_mail(
                            $u['email'],
                            $data['title'],
                            $data['message'],
                            [],
    ...
    public function send_birthday()
    {
        ...
        if (
            !isset($input['user']) ||
            !is_array($input['user']) ||
            count(array_filter($input['user'])) === 0
        ) {
            echo json_encode([
                'status' => 1,
                'msg' => [
                    'user' => 'The Recipient field is required.'
                ]
            ]);
            return;
        }
        
        $this->form_validation->set_rules('birthday_title', 'Title', 'required');
        $this->form_validation->set_rules('birthday_message', 'Message', 'required');
        $this->form_validation->set_rules('birthday_send_by', 'Send Through', 'required');
        ...
        // Send mode
        $send_sms  = ($input['birthday_send_by'] === 'sms') ? 1 : 0;
        $send_mail = ($input['birthday_send_by'] === 'email') ? 1 : 0;
    
        $message       = $input['birthday_message'];
        $message_title = $input['birthday_title'];
        $userlisting   = $input['user'];
    ...
    public function send_group()
    {
        ...
        $this->form_validation->set_rules('send_type', 'Send Type', 'required');
        ...
        if (
            ...
            return $this->output
                ...
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['user' => 'Recipient is required']
                ]));
        }
    
        if ($input['send_type'] === 'schedule' && empty($input['schedule_date_time'])) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['schedule_date_time' => 'Schedule date time is required']
                ]));
        }
            ...
            return $this->output
                ...
                ->set_output(json_encode([
                    ...
                    'msg' => [
                        ...
                        'group_message' => form_error('group_message'),
                        'group_send_by' => form_error('group_send_by'),
                        'send_type'     => form_error('send_type')
                    ]
                ]));
        }
    
        // Send mode
        $send_sms  = ($input['group_send_by'] === 'sms') ? 1 : 0;
        $send_mail = ($input['group_send_by'] === 'email') ? 1 : 0;
        ...
        foreach ($input['user'] as $value) {
            ...
            if ($value === 'student' || $value === 'parent') {
                ...
                foreach ($students as $s) {
                    $user_array[] = [
                        'user_id'  => $s['id'],
                        'email'    => ($value === 'student') ? $s['email'] : $s['guardian_email'],
                        'mobileno' => ($value === 'student') ? $s['mobileno'] : $s['guardian_phone'],
                        'role'     => $value
                    ];
                }
                ...
                $staff = $this->staff_model->getEmployeeByRoleID($value);
                foreach ($staff as $st) {
                    $user_array[] = [
                        'user_id'  => $st['id'],
                        'email'    => $st['email'],
                        'mobileno' => $st['contact_no'],
                        'role'     => 'staff'
                    ];
                }
            }
        ...
        $data = [
            ...
            'message'           => $input['group_message'],
            'send_mail'         => $send_mail,
            'send_sms'          => $send_sms,
            'email_template_id' => $input['template_id'] ?? null,
            'group_list'        => json_encode($input['user']),
            'user_list'         => json_encode($user_array),
            'created_at'        => date('Y-m-d H:i:s')
        ];
    
        // Schedule
        if ($input['send_type'] === 'schedule') {
            $data['is_schedule']        = 1;
            $data['sent']               = 0;
            $data['schedule_date_time'] = date(
                'Y-m-d H:i:s',
    ...
    public function send_group_sms()
    {
        ...
        // 2. Map JSON to $_POST for form_validation
        $_POST = $input;
        ...
        if (
            !isset($input['group_send_by']) ||
            !is_array($input['group_send_by']) ||
            count(array_filter($input['group_send_by'])) === 0
        ) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['group_send_by' => 'Message To']
                ]));
        }
        ...
        // $this->form_validation->set_rules('user', 'Message To', 'required');
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($send_type === 'schedule') {
                $errors['schedule_date_time'] = form_error('schedule_date_time');
            }
        ...
        $message_title = $this->input->post('group_title');
        $message       = $this->input->post('group_message');
        $send_by       = $this->input->post('group_send_by'); // array
        $users         = $this->input->post('user');          // array
        $template_id   = $this->input->post('group_template_id');
        ...
        foreach ($users as $user_type) {
            ...
            if ($user_type === 'student') {
                ...
                $parents = $this->student_model->get();
                foreach ($parents as $parent) {
                    $user_array[] = [
                        'user_id'  => $parent['id'],
                        'email'    => $parent['guardian_email'],
                        'mobileno' => $parent['guardian_phone'],
                        'app_key'  => $parent['parent_app_key'],
                        'role'     => 'parent'
                    ];
                }
    
            } elseif (is_numeric($user_type)) {
    
                $staffs = $this->staff_model->getEmployeeByRoleID($user_type);
        ...
        if ($send_type === 'send_now') {
            ...
            foreach ($user_array as $user) {
                ...
                if (in_array('sms', $send_by) && !empty($user['mobileno'])) {
                    $this->smsgateway->sendSMS(
                        $user['mobileno'],
                        $message,
                        $template_id,
                ...
                if (in_array('push', $send_by) && !empty($user['app_key'])) {
        ...
        $data = [
            ...
            'send_sms'        => 1,
            'send_mail'       => 0,
            'send_through'    => json_encode($send_by),
            'group_list'      => json_encode($users),
            'user_list'       => json_encode($user_array),
            'template_id'     => $template_id,
            'created_at'      => date('Y-m-d H:i:s'),
        ];
        ...
        if ($send_type === 'schedule') {
            ...
            $data['schedule_date_time'] = date(
                'Y-m-d H:i:s',
                $this->customlib->dateTimeformatTwentyfourhour(
                    $this->input->post('schedule_date_time'),
                    false
                )
            );
        }
    ...
    public function send_birthday_sms()
    {
        ...
        // 2. Map JSON to POST for form_validation
        $_POST = $input;
        ...
        if (
            !isset($input['birthday_send_by']) ||
            !is_array($input['birthday_send_by']) ||
            count(array_filter($input['birthday_send_by'])) === 0
        ) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['birthday_send_by' => 'Send Through']
                ]));
        }
        ...
        // 4. Inputs
        $sms_mail      = $this->input->post('birthday_send_by'); // array
        $message       = $this->input->post('birthday_message');
        $message_title = $this->input->post('birthday_title');
        $template_id   = $this->input->post('birthday_template_id');
    
        $userlisting      = $this->input->post('user');     // mobile numbers
        $userpushlisting  = $this->input->post('app_key');  // push keys
    
        $user_array      = [];
        $user_push_array = [];
    
        // 5. Build SMS recipients
        foreach ($userlisting as $mobileno) {
            $user_array[] = [
                'mobileno' => $mobileno
            ];
        }
    
        // 6. Build Push recipients
        if (!empty($userpushlisting)) {
            foreach ($userpushlisting as $app_key) {
                $user_push_array[] = [
                    'app_key' => $app_key
                ];
            }
        ...
        // 7. Send SMS
        if (!empty($user_array) && in_array('sms', $sms_mail)) {
            foreach ($user_array as $user) {
                if (!empty($user['mobileno']) && $user['mobileno'] != 0) {
                    $this->smsgateway->sendSMS(
                        $user['mobileno'],
                        $message,
                        $template_id,
        ...
        if (!empty($user_push_array) && in_array('push', $sms_mail)) {
            ...
            foreach ($user_push_array as $user_push) {
                if (!empty($user_push['app_key'])) {
                    $this->pushnotification->send(
                        $user_push['app_key'],
                        $push_array,
                        'mail_sms'
    ...
    public function send_individual_sms()
    {
        ...
        if (
            !isset($input['user_list']) ||
            !is_array($input['user_list']) ||
            count(array_filter($input['user_list'])) === 0
        ) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['user_list' => 'Recipient']
                ]));
        }
        ...
        if (
            !isset($input['individual_send_by']) ||
            !is_array($input['individual_send_by']) ||
            count(array_filter($input['individual_send_by'])) === 0
        ) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['individual_send_by' => 'Send Through']
                ]));
        }
        ...
        $message_title = $this->input->post('individual_title');
        $message       = $this->input->post('individual_message');
        $sms_mail      = $this->input->post('individual_send_by'); // array
        $template_id   = $this->input->post('individual_template_id');
    
        // user_list comes as JSON array from frontend
        $userlisting = json_decode(json_encode($this->input->post('user_list')));
        ...
        if ($send_type === 'send_now') {
            ...
            foreach ($user_array as $user) {
                ...
                if (in_array('sms', $sms_mail) && !empty($user['mobileno'])) {
                    $this->smsgateway->sendSMS(
                        $user['mobileno'],
                        $message,
                        $template_id,
                ...
                if (in_array('push', $sms_mail) && !empty($user['app_key'])) {
                    $this->pushnotification->send(
                        $user['app_key'],
                        [
                            'title' => $message_title,
    ...
    public function send_class_sms()
    {
        ...
        if (
            !isset($input['class_send_by']) ||
            !is_array($input['class_send_by']) ||
            count(array_filter($input['class_send_by'])) === 0
        ) {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode([
                    'status' => 1,
                    'msg' => ['class_send_by' => 'Send Through']
                ]));
        }
        ...
        $data = [
            ...
            'send_through'     => json_encode($sms_mail),
            'sms_template_id'  => $this->input->post('template_id'),
            'group_list'       => json_encode($sections),
            'template_id'      => $template_id,
            'schedule_class'   => $class_id,
            'schedule_section' => json_encode($sections),
            'send_mail'        => 0,
            'send_sms'         => 1,
            'user_list'        => json_encode($user_array),
            'created_at'       => date('Y-m-d H:i:s'),
        ...
        if ($send_type == 'send_now') {
            ...
            foreach ($user_array as $user) {
                ...
                if (in_array('push', $sms_mail) && !empty($user['app_key'])) {
                    ...
                    $this->pushnotification->send($user['app_key'], $push_data, 'mail_sms');
                }
            }
    ...
    public function send_class()
    {
        ...
        $this->form_validation->set_rules('class_send_by', 'Send Through', 'required');
        $this->form_validation->set_rules('files', 'File', 'callback_multihandle_upload[files]');
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            return $this->response([
                ...
                'errors' => [
                    ...
                    'user'               => form_error('user[]'),
                    'files'              => form_error('files'),
                    'schedule_date_time' => form_error('schedule_date_time'),
                ]
            ], REST_Controller::HTTP_BAD_REQUEST);
        }
    
        // Inputs
        $send_by       = $this->input->post('class_send_by'); // sms / mail
        $message       = $this->input->post('class_message');
        $message_title = $this->input->post('class_title');
        $sections      = $this->input->post('user');
        $class_id      = $this->input->post('class_id');
        ...
        if (!empty($_FILES['files']['name'])) {
            foreach ($_FILES['files']['name'] as $key => $name) {
                ...
                $new_name = time() . rand(100, 999) . '.' . $ext;
    
                move_uploaded_file(
                    $_FILES['files']['tmp_name'][$key],
                    "../uploads/communicate/email_attachments/" . $new_name
                );
        ...
        if ($send_type === 'send_now') {
            ...
            foreach ($user_array as $user) {
                ...
                if ($send_sms && !empty($user['mobileno'])) {
                    $this->smsgateway->sendSMS(
                        $user['mobileno'],
                        '',
                        $message
        ...
        foreach ($attachment_data as $attach) {
            $attach['message_id'] = $message_id;
            $this->messages_model->add_email_attachment($attach);
        }
