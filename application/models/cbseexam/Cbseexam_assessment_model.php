...
class Cbseexam_assessment_model extends MY_Model
{
    ...
    */
    ...
    public function add_graderange($insert_grade, $cbse_exam_grades_range, $cbse_exam_grades_range_update, $delete_grade_range)
    {
        ...
        if (isset($insert_grade['id']) && !empty($insert_grade['id'])) {
            ...
            if (!empty($delete_grade_range)) {
                ...
                $ids = array_column($delete_grade_range, 'id'); // ✅ array

                $this->db->where_in('id', $ids); // ✅ correct
                $this->db->delete('cbse_exam_assessment_types');
            }
    ...
    public function getTypesByAssessmentId($assessment_id)
    {
        return $this->db
            ->where('cbse_exam_assessment_id', $assessment_id)
            ->get('cbse_exam_assessment_types')
            ->result_array();
    }

    public function check_assessment_name($name, $id = null)
    {
        ...
        return $this->db->get('cbse_exam_assessments')->row();

        // echo $this->db->last_query();   exit;
    }

    public function check_type_duplicate($name, $code, $id = null)
    {
        $this->db->group_start()
            ->where('name', $name)
            ->or_where('code', $code)
        ...
        return $this->db->get('cbse_exam_assessment_types')->row();
    }
}
