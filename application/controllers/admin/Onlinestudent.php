...
class Onlinestudent extends Public_Controller
{
    ...
    public function delete()
    {
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status'  => true,
                'message' => 'Online admission record deleted successfully'
            ]));
    }
    ...
    public function get_edit_student_details($id)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
            exit;
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'data' => [
                    ...
                    'settings'     => $this->sch_setting_detail
                ]
            ]));
    }
    ...
    public function edit()
    {
        ...
        $saveType = $this->input->post('save'); // save | enroll
    
        $response = $this->onlinestudent_model->update(
            $data,
            $this->input->post('fee_session_group_id'),
            $this->input->post('transport_feemaster_id'),
            $saveType
        );
    ...
    public function getstudentlist()
    {
        ...
        // Fetch students
        $student_result = $this->onlinestudent_model->getstudentlist($class_ids, null);
        $result         = json_decode($student_result);
        ...
        if (!empty($result->data)) {
            foreach ($result->data as $student) {
                ...
                $students[] = [
                    ...
                    'name'          => trim(
                        $student->firstname . ' ' .
                        ($sch_setting->middlename ? $student->middlename . ' ' : '') .
                        ($sch_setting->lastname ? $student->lastname : '')
                    ),
                    'class'         => $student->class,
                    'section'       => $student->section,
                    'father_name'   => $sch_setting->father_name ? $student->father_name : null,
                    'dob'           => $student->dob
                                        ? $this->customlib->dateformat($student->dob)
                                        : null,
                    'gender'        => strtolower($student->gender),
                    'category'      => $student->category,
                    'mobile'        => $sch_setting->mobile_no ? $student->mobileno : null,
    
                    'form_status'   => $student->form_status == 1 ? 'submitted' : 'not_submitted',
                    'payment_status'=> $sch_setting->online_admission_payment == 'yes'
                                        ? ($student->paid_status == 1 ? 'paid' :
                                           ($student->paid_status == 2 ? 'processing' : 'unpaid'))
                                        : null,
    
                    'is_enrolled'   => (bool) $student->is_enroll,
                    'submitted_at'  => $student->submit_date
                                        ? $this->customlib->dateformat($student->submit_date)
                                        : null,
                    'created_at'    => $this->customlib->dateformat(
    ...
    public function checkPaymentStatus()
    {
        ...
        if (
            $result['form_status'] != 1 &&
            $sch_setting->online_admission_payment == 'yes' &&
            $result['paid_status'] == 0
        ) {
    
            $message =
                $this->lang->line('form_status') . " : " . $this->lang->line('not_submitted') . "\n" .
                $this->lang->line('payment_status') . " : " . $this->lang->line('unpaid') . "\n\n" .
                $this->lang->line('do_you_still_want_to_enroll_it');
    
        } elseif (
            $result['form_status'] != 1 &&
            $sch_setting->online_admission_payment == 'no'
        ) {
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'data' => [
                    ...
                    'payment_enabled'=> $sch_setting->online_admission_payment,
                    'show_confirm'   => !empty($message),
                    'message'        => $message
                ]
            ]));
    }
