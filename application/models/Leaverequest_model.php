...
class Leaverequest_model extends MY_model
{
    ...
    {
        $id_e = $this->db->escape($id);
        $leave_type_id_e = $this->db->escape($leave_type_id);
        
        $query = $this->db->select('staff_leave_details.*,leave_types.type,leave_types.id as typeid , (SELECT sum(leave_days) from staff_leave_request WHERE leave_type_id=' . $leave_type_id_e . ' and staff_id=' . $id_e . ' and status !="disapprove") as `total_applied`', null, false)->where(array('staff_id' => $id, 'leave_types.id' => $leave_type_id))->join("leave_types", "staff_leave_details.leave_type_id = leave_types.id")->get("staff_leave_details");
