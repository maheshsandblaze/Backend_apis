...
class Welcome extends Public_Controller
{
    ...
    public function online_admission_review($reference_no = null)
    {
        ...
        // Session validation
        // $admin_session = $this->session->userdata('admin');
        // $status = !empty($admin_session) ? 'admin' : 'student';
        ...
        $classresult = $this->onlinestudent_model
            ->getclassbyclasssectionid($result['class_section_id']);
        ...
        $response = [
            ...
            'contact' => [
                ...
                'email'   => $result['email'],
                'address' => [
                    'current'   => $result['current_address'],
                    'permanent' => $result['permanent_address'],
                ]
            ],
    
            // Parent Info
            'father' => [
                'name'        => $result['father_name'],
                'phone'       => $result['father_phone'],
                'occupation'  => $result['father_occupation'],
                'image'       => $result['father_pic'],
            ],
            'mother' => [
                'name'        => $result['mother_name'],
                'phone'       => $result['mother_phone'],
            ...
            // Guardian Info
            'guardian' => [
                'is'          => $result['guardian_is'],
                'name'        => $result['guardian_name'],
                'relation'    => $result['guardian_relation'],
                'phone'       => $result['guardian_phone'],
                'email'       => $result['guardian_email'],
                'occupation'  => $result['guardian_occupation'],
                'address'     => $result['guardian_address'],
            ],
    
            // Bank & IDs
            'bank' => [
                'account_no' => $result['bank_account_no'],
                'bank_name'  => $result['bank_name'],
                'ifsc'       => $result['ifsc_code'],
            ],
            'documents' => [
                'aadhaar' => $result['adhar_no'],
                'samagra' => $result['samagra_id'],
            ],
    
            // Payment
            'transaction' => [
                'transaction_id'   => $this->customlib->gettransactionid($id),
                'paid_amount'      => $this->customlib->gettransactionpaidamount($id),
                'admission_amount' => $this->sch_setting_detail->online_admission_amount,
                'payment_enabled'  => $this->sch_setting_detail->online_admission_payment,
            ],
    ...
    public function printmarksheet($student_session_id, $template_id)
    {

        $this->load->model(array('cbseexam/cbseexam_exam_model', "cbseexam/cbseexam_assessment_model", 'cbseexam/cbseexam_result_model','student_model'));
        ...
        $setting_data                 = $this->setting_model->get();
        $this->data['setting_data']     = $setting_data;
        $this->data['student_id']  = $student_session_id;
        $cbse_template = $this->cbseexam_result_model->get_cbse_template($template_id);
        $student_data = $this->student_model->getByStudentSession($student_session_id);


        $this->data['template_data'] = $cbse_template;
        $this->data['student'] = $student_data;

        // echo "<pre>";print_r($student_data);exit;



        $this->load_theme('pages/indexview', $this->config->item('front_layout'));
    }
}
