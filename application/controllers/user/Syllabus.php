...
class Syllabus extends Public_Controller
{
    ...
    public function index()
    {
        ...
        $formatted_start = date(
            ...
            $this->customlib->dateyyyymmddTodateformat($this_week_start)
        );

        $formatted_end = date(
            $this->customlib->getSchoolDateFormat(),
            $this->customlib->dateyyyymmddTodateformat($this_week_end)
        );
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status' => true,
                'data'   => [
                    'this_week_start' => $formatted_start,
                    'this_week_end'   => $formatted_end
                ]
            ]));
    }
    ...
    public function get_weekdates()
    {
        ...
        $student_data = $this->syllabus_model->get_studentsyllabusdata($student_current_class);
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status' => true,
                'data' => [
                    'this_week_start' => $this->customlib->dateformat($this_week_start),
                    'this_week_end'   => $this->customlib->dateformat($this_week_end),
                    'prev_week_start' => $this->customlib->dateformat($prev_week_start),
                    'next_week_start' => $this->customlib->dateformat($next_week_start),
    ...
    public function get_subject_syllabus()
    {
        ...
        // ===============================
        $input = json_decode(file_get_contents("php://input"), true);
        $subject_syllabus_id = $input['subject_syllabus_id'] ?? '';

        if (empty($subject_syllabus_id)) {
            return $this->output
                ->set_status_header(400)
                ->set_output(json_encode([
                    'status' => false,
                    'message' => 'subject_syllabus_id is required'
                ]));
        }

        $result = $this->syllabus_model
            ->get_subject_syllabus_student([
