		...
		if (array_key_exists($exam_id, $exams)) {
			...
			foreach ($exams[$exam_id]['subjects'] as $subjects_key => $subjects_value) {
				...
				foreach ($subjects_value['exam_assessments'] as $subject_key => $subject_value) {
					...
					if($subjects_value['subject_name'] == "Physical Science" || $subjects_value['subject_name'] == "Biology")
					{
						$maximum_marks = $maximum_marks + $subject_value['maximum_marks'] / 2 ;
						$subject_max_marks = $subject_max_marks + $subject_value['maximum_marks'] / 2;
