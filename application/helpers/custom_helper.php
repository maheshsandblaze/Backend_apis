...
if (!function_exists('is_active_2fa')) {
    ...
    {
        $CI = &get_instance();
        $CI->load->model(array("google_authenticator/gauth_model", "studentfeemaster_model"));
...
function getstudentFeesdue($student_session_id)
{
    ...
    $totalbalance = 0;
    $student_total_fees = $CI->studentfeemaster_model->getStudentFees($student_session_id);
    // echo "<pre>";
    ...
    if (!empty($student_total_fees)) {
        ...
        $totalbalance     = $totalfee - ($deposit + $discount);

        // print_r($totalbalance);exit;

        // echo $totalbalance; echo "<br>";
