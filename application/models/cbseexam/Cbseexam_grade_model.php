...
class Cbseexam_grade_model extends MY_Model
{
    ...
    */
    ...
    public function getRangesByGradeId($grade_id)
    {
        return $this->db
            ->where('cbse_exam_grade_id', $grade_id)
            ->get('cbse_exam_grades_range')
            ->result_array();
    }

    public function check_grade_name($name, $id = null)
    {
        $this->db->where('name', $name);

        if (!empty($id)) {
            $this->db->where('id !=', $id); // exclude current record (UPDATE case)
        }

        return $this->db->get('cbse_exam_grades')->row();
    }
