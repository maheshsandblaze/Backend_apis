...
class Visitor_management extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array("classteacher_model",'class_section_time_model','visitor_management_model','student_model'));
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/visitor_management');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $visitorlist             = $this->visitor_management_model->get_today_entries();
            // echo "<pre>";
            // print_r($visitorlist);exit;
            $data['late_entries'] = $visitorlist;
            $this->load->view('layout/header', $data);
            $this->load->view('admin/visitor_management/late_entriesList', $data);
            ...
            if(!empty($studentData))
            {
                ...
                if (!empty($_FILES['file']['name'])) { // If a file is uploaded
                    $img_name = $this->media_storage->fileupload("file", "./uploads/front_office/visitors/");
                } else {
                    $img_name = ""; // No file uploaded, save an empty value
                }
                ...
                $insert_data = array(
                    ...
                    'visitor_relation'      =>  $this->input->post('visitor_relation'),
                    'purpose'               =>  $this->input->post('visitorpurpose'),
               ...
               $insert_id     = $this->visitor_management_model->add($insert_data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            if ($this->input->is_ajax_request()) {
                ...
                $lateEntries = $this->visitor_management_model->get_today_entries();
        ...
        if (!empty($stuData)) {
            $result = array(
                ...
                'studentData' => array(
                    ...
                    'admission_no' => $admission_no,
                    'student_session_id' => $stuData->student_session_id,
                    'father_name' => $stuData->father_name,
                    'father_phone' => $stuData->father_phone,
                    'mother_name' => $stuData->mother_name,
                    'mother_phone' => $stuData->mother_phone,
                    'guardian_relation' => $stuData->guardian_relation,
                    'father_pic' => !empty($stuData->father_pic) ? $this->media_storage->getImageURL($stuData->father_pic) : null,
                    'mother_pic' => !empty($stuData->mother_pic) ? $this->media_storage->getImageURL($stuData->mother_pic) : null,
                    'guardian_pic' => !empty($stuData->guardian_pic) ? $this->media_storage->getImageURL($stuData->guardian_pic) : null
                ),
            );
        } else {
            $result = array(
