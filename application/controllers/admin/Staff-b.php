...
class Staff extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        // echo '<pre>';print_r($staffRole);exit;
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $is_invisible_user            = $this->input->post("is_invisible_user");
            ...
            $data1 = array(
                ...
                'linkedin'             => $linkedin,
                'instagram'            => $instagram,
                'is_invisibles_user' => $is_invisible_user
            );
    ...
    public function recruitment()
    {
        ...
        $data['recruitmentresult'] = $recruitmentresult;
        //print_r( $data['recruitmentresult']);exit;
        if ($this->input->server('REQUEST_METHOD') === 'POST') {

            $data = array(
                'name' => $this->input->post('name', TRUE),
                'position' => $this->input->post('position', TRUE),
                'openings' => $this->input->post('no_of_openings', TRUE),
            ...
            $this->session->set_flashdata('success', 'Recruitment data added successfully.');

            redirect('admin/staff/recruitment');
        }

        $this->load->view('layout/header');
        $this->load->view('admin/staff/staff_recruitment', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function g_category_update()
    {
        ...
        if (!empty($category_id) && !empty($category_name)) {
            ...
            $this->admin_model->update_category($category_id, $data_update);
    ...
    public function gallery_list($id)
    {
        ...
        $data['category_id'] = $id;
        $data['gallery_list'] = $this->admin_model->get_gallery_by_category($id);
    ...
    public function add_gallery_image()
    {
        ...
        if (!is_dir($upload_dir)) {
            if (!mkdir($upload_dir, 0777, true) && !is_dir($upload_dir)) {
                $this->session->set_flashdata('error', "Failed to create directory: " . $upload_dir);
                redirect('admin/staff/gallery_list/' . $category_id);
            }
        }

        $image_path = '';
        if (isset($_FILES["category_image"]) && !empty($_FILES['category_image']['name'])) {
            $image_path = $this->media_storage->fileupload("category_image", $upload_dir);
        }
        ...
        if (!empty($image_path)) {
            ...
            $this->admin_model->insert_gallery_image($insertData);
