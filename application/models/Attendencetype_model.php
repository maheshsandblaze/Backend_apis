...
class Attendencetype_model extends CI_Model
{
    ...
    // New clone method
    public function get_attendence_type($id = null)
    {
        $this->db
            ->select('attendence_type.type, attendence_type.id as attendence_type_id, attendence_type.key_value, attendence_type.is_active, attendence_type.created_at, attendence_type.updated_at')
            ->from('attendence_type');
    
        if ($id !== null) {
            $this->db->where('attendence_type.id', $id);
        } else {
            $this->db->where('attendence_type.is_active', 'yes');
            $this->db->order_by('attendence_type.id', 'ASC');
        }
