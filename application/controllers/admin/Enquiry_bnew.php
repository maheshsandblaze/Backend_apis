...
class Enquiry extends Admin_Controller
{
    ...
    public function feefollow_up($enquiry_id, $status, $created_by)
    {
        ...
        $data['id']              = $enquiry_id;
        [$enquiry_data] = $this->feeenquiry_model->get($enquiry_id);
        $data['enquiry_data'] = $enquiry_data;
        $student_data = $this->student_model->get($data['enquiry_data']['student_id']);
        ...
        $data['login_staff_id']  = $login_staff_id;
        $data['superadmin_rest'] = $this->session->userdata['admin']['superadmin_restriction'];
        $this->load->view('admin/fee_enquiry/follow_up_modal', $data);
    }
    ...
    public function fee_follow_up_insert()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $follow_up = array(
                ...
                'note'        => $this->input->post('note'),
                'followup_by' => $staff_id,
                'feeenquiry_id'  => $this->input->post('enquiry_id'),
            );
    ...
    public function fee_enquiry($student_id, $agent_id, $feetype_id)
    {
        ...
        if ($row_count == 0) {
            ...
            if ($insert_id) {
                [$row] = $this->feeenquiry_model->get(null, $student_id, $feetype_id);
                $data['enquiry_data'] = $row;
                $data['assigned_staff'] = '';  
                if(!empty($data['enquiry_data']['assigned'])){
                    $data['assigned_staff'] = $this->staff_model->get($data['enquiry_data']['assigned']);
                }
                $data['created_by']      = $this->staff_model->get($created_by);
                $page = $this->load->view('admin/fee_enquiry/follow_up_modal', $data, true);
                echo json_encode(array('enquiry_id' => $insert_id, 'page' => $page));
            }
        } else {
            [$row] = $this->feeenquiry_model->get(null, $student_id,  $feetype_id);
            $data['enquiry_data'] = $row;
            ...
            $data['created_by']      = $this->staff_model->get($agent_id);
            $page = $this->load->view('admin/fee_enquiry/follow_up_modal', $data, true);
            echo json_encode(array('enquiry_id' =>  $row['id'], 'page' => $page));
        }
    }
    ...
    {
        $data['id']             = $id;
        $data['follow_up_list'] = $this->feefollowup_model->getfollow_up_list($id);
        $this->load->view('admin/fee_enquiry/followuplist', $data);
    }
    ...
    public function fee_change_status()
    {
        ...
        if (!empty($id)) {
            $data = array('id' => $id, 'status' => $status);
            $this->feeenquiry_model->changeStatus($data);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        } else {
            $array = array('status' => 'fail', 'error' => '', 'message' => $this->lang->line('update_message'));
        }
    ...
    public function admissions_analysis_report()
    {
        ...
        $tot_leads          = $this->enquiry_model->getTotalLeads();
        if (!empty($tot_leads)) {
            $total_leads = $tot_leads->total_leads;
        }
        
        $tot_passive        = $this->enquiry_model->getTotalPassiveLeads();
        if (!empty($tot_passive)) {
            $total_passive = $tot_passive->total_passive;
        }
        
        $tot_won        = $this->enquiry_model->getTotalWonLeads();
        if (!empty($tot_won)) {
            $total_won = $tot_won->total_won;
        }
        
        $tot_dead        = $this->enquiry_model->getTotalDeadLeads();
        if (!empty($tot_dead)) {
            $total_dead = $tot_dead->total_dead;
        }

        $data['total_leads'] = $total_leads;
        $data['total_passive'] = $total_passive;
        $data['total_won'] = $total_won;
        $data['total_dead'] = $total_dead;
        
        $this->load->view('layout/header', $data);
        $this->load->view('admin/frontoffice/admissionsanalysis', $data);
