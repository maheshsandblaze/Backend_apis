...
class Report extends Public_Controller
{
    ...
    public function admission_report_mobile()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['resultlist'] = $this->student_model->admission_report_mobile($searchterm, $carray, $condition);
        }
    ...
    public function get_sibling_report()
    {
        // Preflight
    ...
    public function sibling_report()
    {
        ...
        // Step 1: Find parents with siblings
        $sibling_list = $this->student_model->sibling_reportsearch(
            $searchterm,
            $carray = null,
            $condition
        );
        ...
        $sibling_parent_ids = [];
        foreach ($sibling_list as $row) {
            $sibling_parent_ids[] = $row['parent_id'];
        }
    
        // Step 2: Fetch full sibling list
        $resultlist = $this->student_model->sibling_report($searchterm, $carray = null);
    
        $siblings = [];
        foreach ($resultlist as $row) {
            if (in_array($row['parent_id'], $sibling_parent_ids)) {
                $siblings[$row['parent_id']][] = $row;
            }
        }
    ...
    public function studentinformation()
    {
        ...
        // No data processing here – just API availability
        return $this->output
            ->set_status_header(200)
            ->set_content_type('application/json')
            ->set_output(json_encode([
                'status'  => true,
                'message' => 'Student Information Report API is available'
            ]));
    }
    ...
    // public function studentmasterreport()
    // {
    //     if (!$this->rbac->hasPrivilege('student_report', 'can_view')) {
    //         access_denied();
    //     }
    ...
    //     $this->session->set_userdata('top_menu', 'Reports');
    //     $this->session->set_userdata('sub_menu', 'Reports/student_information');
    //     $this->session->set_userdata('subsub_menu', 'Reports/student_information/student_master_report');
    //     $data['title']           = 'student fee';
    //     $genderList              = $this->customlib->getGender();
    //     $data['genderList']      = $genderList;
    //     $RTEstatusList           = $this->customlib->getRteStatus();
    ...
    //     $category                = $this->category_model->get();
    //     $data['categorylist']    = $category;
    //     $student_data            = $this->student_model->getstudentmaster();
    //     $data['student_data']    = $student_data;
    //     $this->load->view('layout/header', $data);
    //     $this->load->view('reports/studentMasterReport', $data);
    //     $this->load->view('layout/footer', $data);
    // }
    ...
    public function studentmasterreport()
    {
        ...
        $data['RTEstatusList']    = $this->customlib->getRteStatus();
        $data['classlist']        = $this->class_model->get();
        $data['sch_setting']      = $this->sch_setting_detail;
        $data['adm_auto_insert']  = $this->sch_setting_detail->adm_auto_insert ?? null;
        $data['categorylist']     = $this->category_model->get();
        $data['student_data']     = $this->student_model->getstudentmaster();
    ...
    public function classsectionreport()
    {
        ...
        // Fetch class & section report data
        $data = [];
        $data['title']              = 'Class & Section Report';
        $data['class_section_list'] = $this->classsection_model->getClassSectionStudentCount();
    ...
    public function get_guardianreport()
    {
        // Preflight
        if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
            exit;
        ...
        $data = [
            'title'           => 'Student Guardian Report',
            'classlist'       => $this->class_model->get(),
            'sch_setting'     => $this->sch_setting_detail,
            'adm_auto_insert' => $this->sch_setting_detail->adm_auto_insert ?? null
        ];
    ...
    public function guardianreport()
    {
        ...
        // Validation
        $errors = [
            'class_id'   => empty($class_id) ? 'Class is required' : null,
            'section_id' => empty($section_id) ? 'Section is required' : null
        ];
    ...
    public function dtcredentialreportlist()
    {
        ...
        if (!empty($resultlist->data)) {
            foreach ($resultlist->data as $student) {
                ...
                $row[] = $studentlist['username'] ?? '';
                $row[] = $studentlist['password'] ?? '';
    ...
    // public function student_all_data_report()
    // {
    //     if (!$this->rbac->hasPrivilege('student_all_data_report', 'can_view')) {
    //         access_denied();
    //     }    
   
    //     $this->session->set_userdata('top_menu', 'Reports');
    //     $this->session->set_userdata('sub_menu', 'Reports/student_information');
    //     $this->session->set_userdata('subsub_menu', 'Reports/student_information/student_all_data_report');
    //     $data['title']           = 'Student All Data Report'; 

    //     // $students = $this->student_model->getStudentAllData();
    ...
    //     $this->load->view('layout/header', $data);
    //     $this->load->view('reports/student_all_data_report', $data);
    //     $this->load->view('layout/footer', $data);
    ...
    //     $this->session->set_userdata('top_menu', 'Reports');
    //     $this->session->set_userdata('sub_menu', 'Reports/student_information');
    //     $this->session->set_userdata('subsub_menu', 'Reports/student_information/app_install_users');
    //     $data['searchlist']      = $this->search_type;
    //     $data['sch_setting']     = $this->sch_setting_detail;
    //     $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;

    //     $data['resultlist'] = $this->student_model->app_install_report();

    //     $this->load->view('layout/header', $data);
    //     $this->load->view('reports/app_install_report', $data);
    //     $this->load->view('layout/footer', $data);
    // }
    ...
    public function app_install_report()
    {
        ...
        // Fetch report data
        $resultlist = $this->student_model->app_install_report();
