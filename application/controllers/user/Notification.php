...
class Notification extends Public_Controller
{
    ...
    public function index()
    {
        ...
        if ($user_role === 'student') {
            $notifications =
                $this->notification_model
                     ->getNotificationForStudent($user_id);
    
        } elseif ($user_role === 'parent') {
            $notifications =
                $this->notification_model
                     ->getNotificationForParent($user_id);
        } else {
            $notifications = [];
        }
        ...
        foreach ($notifications as $row) {
            if (strtotime(date('Y-m-d')) >= strtotime($row['publish_date'])) {
                $notification_bydate[] = $row;
            }
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status' => true,
                'role'   => $user_role,
                'count'  => count($notification_bydate),
                'data'   => $notification_bydate
            ]));
    }
