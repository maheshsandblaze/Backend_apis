...
class Stuattendence_model extends MY_Model
{
    ...
    public function add($insert_array, $update_array)
    {
        ...
        if (!empty($insert_array)) {
            $filtered_insert_array = [];

            foreach ($insert_array as $attendance) {
                ...
                if ($query->num_rows() == 0) {
                    $filtered_insert_array[] = $attendance;
                }
            }

            if (!empty($filtered_insert_array)) {
                $this->db->insert_batch('student_attendences', $filtered_insert_array);
            }
        }
    ...
    public function delete_attedances($attedance_id)
    {

        if ($attedance_id != 0) {

            $this->db->where_in('id', $attedance_id);
