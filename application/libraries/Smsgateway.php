...
class Smsgateway
{
    ...
    public function sendWhatsapp($send_to, $detail, $message_type, $video)
    {
        if ($message_type == 'text') {
            $params=array(
                'token' => 'q42r4tozsswl44md',
                'to' => $send_to,
                'body' => $detail
                );
                $curl = curl_init();
                curl_setopt_array($curl, array(
                  CURLOPT_URL => "https://api.ultramsg.com/instance89912/messages/chat",
        ...
        if ($message_type == 'video') {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
              CURLOPT_URL => "https://api.ultramsg.com/instance89912/messages/video",
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 30,
...
//            }
    ...
    public function getGroupAddFeeContent($data, $template, $sms_detail_type = null)
    {
        ...
        $data['class']        = $fee->class;
        $data['section']      = $fee->section;
        $data['fee_amount']   = amountFormat($fee_amount);
        $data['student_name'] = $this->_CI->customlib->getFullName($fee->firstname, $fee->middlename, $fee->lastname, $this->sch_setting[0]['middlename'], $this->sch_setting[0]['lastname']); 
