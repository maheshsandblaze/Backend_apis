...
class Stuattendence extends Public_Controller
{
    ...
    public function search_students_attendence()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
            exit;
        }
        ...
        $attendencetypeslist    = $this->attendencetype_model->get_attendence_type();
    
        $data = $this->stuattendence_model
            ->searchAttendenceClassSection(
                $input['class_id'],
                $input['section_id'],
                $date
            );
    ...
    public function save_student_attendence()
    {
        ...
        /* =========================
           DATE FIX (STRING → Y-m-d)
        ========================== */
        $date = date(
            'Y-m-d',
            strtotime($input['date']) // handles 18-01-2026 safely
        );
        ...
        /* =========================
           LOOP STUDENTS
        ========================== */
        foreach ($input['students'] as $stu) {
    
            if (
                empty($stu['student_session_id']) ||
                empty($stu['attendance_type_id'])
            ) {
            ...
            $row = [
                ...
                'attendence_type_id'   => $stu['attendance_type_id'],
                'remark'               => $stu['remark'] ?? '',
                'date'                 => $date
            ];
    
            if (!empty($stu['attendance_id'])) {
                $row['id'] = $stu['attendance_id'];
                $update[] = $row;
            } else {
                $insert[] = $row;
            }
        ...
        if (!empty($insert) || !empty($update)) {
            $this->stuattendence_model->add($insert, $update);
        }
    ...
    //     function delete_bulk_attedance()
    // {
    ...
    //         foreach($record_array as $receipt)
    //         {
                
    //             $attendance_ids = $receipt['attedanace_ids'];

    //             $this->stuattendence_model->delete_attedances($attendance_ids);
    ...
    public function delete_bulk_attendence()
    {
        // Handle preflight
        if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
            exit;
        ...
        foreach ($records as $receipt) {
            if (!empty($receipt['attendence_ids'])) {
                $attendance_ids = $receipt['attendence_ids'];
                $this->stuattendence_model->delete_attedances($attendance_ids);
            }
        }
