...
class Notification_model extends MY_Model
{
     ...
     * @return mixed
     */
    public function get($id = null, $role_id)
    {
    ...
    public function getUnreadStudentNotification($student_session_id)
    {

        $sql   = "select send_notification.* from send_notification  left JOIN read_notification on  read_notification.student_id=" . $student_session_id . " and read_notification.notification_id = send_notification.id WHERE  send_notification.visible_student='yes' and read_notification.id IS NULL  group by send_notification.id order by send_notification.id desc";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' order by send_notification.publish_date desc");
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $message   = INSERT_RECORD_CONSTANT . " On  send notification id " . $record_id;
            $action    = "Insert";
        }
        $this->log($message, $record_id, $action);
        //======================Code End==============================
