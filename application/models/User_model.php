...
class User_model extends MY_Model
{
    ...
    // FIND_IN_SET(" . $this->db->escape_str($class_id) 

    public function checkLoginNew($data)
    {
        ...
        $this->db->from('users');
        // $this->db->join('students', 'students.id = users.user_id');         
        $this->db->where('FIND_IN_SET("' . $data['username'] . '", users.username)');
        if (!empty($student_login) && in_array("admission_no", $student_login)) {
            $this->db->or_where('students.admission_no', $data['username']);
        }
        if (!empty($student_login) && in_array("mobile_number", $student_login)) {
        ...
        $stored_passwords = explode(',', $result[0]->password);
        $stored_usernames = explode(',', $result[0]->username);
        // echo "<pre>";
        // print_r($stored_passwords);exit;



       
        if (in_array($data['password'], $stored_passwords))
        {
    ...
    public function get_gallery_by_category($id)
    {
        $this->db->where('category_id', $id);
        $query = $this->db->get('gallery_list');
