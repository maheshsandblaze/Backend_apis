...
class Schsettings extends Public_Controller
{
    ...
    // Upload the print logo API Start
    // public function uploadschollLogo()
    // {
    //     // Allow only POST
    ...
    public function uploadschollLogo()
    {
        ...
        // ----------------------------
        // ABSOLUTE UPLOAD PATH (FIX)
        // ----------------------------
        $upload_path = FCPATH . '../uploads/school_content/logo/';
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $upload = $this->media_storage->fileupload('file', $upload_path);
            ...
            // Get filename safely
            $img_name = is_array($upload) ? $upload['file_name'] : $upload;

            // Delete old logo
            if (!empty($setting->image)) {
                $this->media_storage->filedelete(
                    $setting->image,
                    FCPATH . '../uploads/school_content/logo'
                );
            }
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'message'   => 'Logo updated successfully',
                'image'     => $img_name,
                'image_url' => base_url('../uploads/school_content/logo/' . $img_name)
            ]));
    }



    // Upload the print logo API End
    ...
    // Upload the admin small logo API Start
    public function uploadAdminSmallLogo()
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return $this->output
                ->set_status_header(405)
        ...
        // ✅ ROOT upload path
        $relativePath = '../uploads/school_content/admin_small_logo/';
        ...
        $img_name = $this->media_storage->fileupload('file', $path);

        if (!empty($setting->admin_small_logo)) {
            $this->media_storage->filedelete(
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                'status'  => true,
                'message' => 'Admin small logo updated successfully',
                'image'   => base_url($relativePath . $img_name)
            ]));
    }

    // Upload the admin small logo API End
    ...
    // Upload the admin logo API Start
    public function uploadAdminLogo()
    {
        ...
        $relativePath = '../uploads/school_content/admin_logo/';
        $path = FCPATH . $relativePath;
    ...
    public function generalsetting()
    {
        ...
        /* =======================
           TOKEN VALIDATION
        ======================== */
        // $auth_user = $this->admin_model->validate_token();
        ...
        $_POST = $input; // for form_validation compatibility

        /* =======================
           VALIDATION RULES
        ======================== */
        $this->form_validation->set_rules('currency_format', 'Currency Format', 'required|trim');
        $this->form_validation->set_rules('sch_session_id', 'Session', 'required|trim');
        $this->form_validation->set_rules('sch_name', 'School Name', 'required|trim');
        $this->form_validation->set_rules('sch_phone', 'Phone', 'required|trim');
        $this->form_validation->set_rules('sch_start_month', 'Start Month', 'required|trim');
        $this->form_validation->set_rules('sch_address', 'Address', 'required|trim');
        $this->form_validation->set_rules('sch_email', 'Email', 'required|trim');
        $this->form_validation->set_rules('sch_timezone', 'Timezone', 'required|trim');
        $this->form_validation->set_rules('currency_place', 'Currency Place', 'required|trim');
        $this->form_validation->set_rules('sch_date_format', 'Date Format', 'required|trim');
        $this->form_validation->set_rules('sch_start_week', 'Start Week', 'required|trim');
        $this->form_validation->set_rules('base_url', 'Base URL', 'required|trim');
        $this->form_validation->set_rules('folder_path', 'Folder Path', 'required|trim');
        ...
        ======================== */
        $setting = $this->setting_model->getSetting();
        $sch_id  = $setting->id ?? null;

        $data = [
            'id'              => $sch_id,
            'session_id'      => $input['sch_session_id'],
            'name'            => $input['sch_name'],
            'phone'           => $input['sch_phone'],
            'dise_code'       => $input['sch_dise_code'] ?? null,
            'start_month'     => $input['sch_start_month'],
            'start_week'      => $input['sch_start_week'],
            'address'         => $input['sch_address'],
            'email'           => $input['sch_email'],
            'timezone'        => $input['sch_timezone'],
            'date_format'     => $input['sch_date_format'],
            'currency_format' => $input['currency_format'],
            'currency_place'  => $input['currency_place'],
    ...
    // Upload the app logo API Start
    public function uploadAppLogo()
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return $this->output
                ->set_status_header(405)
        ...
        $relativePath = '../uploads/school_content/logo/app_logo/';
        $path = FCPATH . $relativePath;
    ...
    // Upload the app logo API End


    //Get sessions data API starts
    public function getSessions($id = null)
    {
        ...
        $session_result = $this->session_model->get($id);
    ...
    //Get sessions data API end


    public function getAllSessionwithactive()
    {
        // Allow only GET
        if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
            return $this->output
    ...
    public function savemobileapp_Api()
    {
        // Allow only POST
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return $this->output
        ...
        /* =======================
           VALIDATION RULES
        ======================== */
        $this->form_validation->set_rules('sch_id', 'School ID', 'required|trim');
        $this->form_validation->set_rules('mobile_api_url', 'Mobile API URL', 'required|trim');
        // $this->form_validation->set_rules('admin_mobile_api_url', 'Admin Mobile API URL', 'required|trim');
        ...
        $data = [
            'id'                             => $input['sch_id'],
            'mobile_api_url'                 => $input['mobile_api_url'],
            'app_primary_color_code'         => $input['app_primary_color_code'] ?? null,
            'app_secondary_color_code'       => $input['app_secondary_color_code'] ?? null,
            'admin_app_primary_color_code'   => $input['admin_app_primary_color_code'] ?? null,
            'admin_app_secondary_color_code' => $input['admin_app_secondary_color_code'] ?? null,
            'admin_mobile_api_url'           => $input['admin_mobile_api_url'],
        ];
    ...
    public function idautogeneration()
    {
        ...
        $digitList = $this->customlib->getDigits();

        $setting = $this->setting_model->getSetting();
        $setting->base_url    = empty($setting->base_url) ? base_url() : $setting->base_url;
        $setting->folder_path = empty($setting->folder_path) ? FCPATH : $setting->folder_path;
    ...
    public function saveidautogeneration()
    {
        ...
        if ($this->input->post('adm_auto_insert')) {
            $this->form_validation->set_rules('adm_prefix', 'Admission Prefix', 'trim|required');
            $this->form_validation->set_rules('adm_start_from', 'Admission Start From', 'trim|required|integer');
            $this->form_validation->set_rules('adm_no_digit', 'Admission Digit', 'trim|required|integer|callback_check_admission_digit');
        }

        if ($this->input->post('staffid_auto_insert')) {
            $this->form_validation->set_rules('staffid_prefix', 'Staff Prefix', 'trim|required');
            $this->form_validation->set_rules('staffid_start_from', 'Staff Start From', 'trim|required|integer');
            $this->form_validation->set_rules('staffid_no_digit', 'Staff Digit', 'trim|required|integer|callback_check_staff_id_digit');
        }
    ...
    public function add_admin_user_login_background()
    {
        // Allow only POST
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            return $this->output
        ...
        ======================== */
        $id        = $this->input->post('id');
        $logo_type = $this->input->post('logo_type'); // admin_logo | user_logo
        ...
        if ($logo_type !== 'admin_logo') {
            $data_record = [
                'id' => $id,
                'user_login_page_background' => $img_name
            ];
        } else {
            $data_record = [
                'id' => $id,
                'admin_login_page_background' => $img_name
            ];
        }
