                                    ...
                                    <?php foreach ($result as $key => $value) { ?>
                                        <tr>
                                            ...
                                            <td class="text-right white-space-nowrap">
                                                ...
                                                <a href="<?php echo site_url('cbseexam/exam/examwiseadmitcard/' . $value['id']); ?>" data-toggle="tooltip" title="" class="btn btn-default btn-xs" id="load" data-examid="<?php echo $value['id']; ?>" data-original-title="Print Hall Ticket"><i class="fa fa-ticket"></i></a>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="examheight100 relative">
                    ...
                    <?php  if ($this->rbac->hasPrivilege('cbse_exam_bulk_upload', 'can_view')) { ?>


                    <div class="pull-right box-tools">


                        <a href="<?php echo site_url('student/exportsubjectstudentmarks') ?>">

                            <button class="btn btn-primary btn-sm" id="downloadButton"><i class="fa fa-download"></i> <?php echo $this->lang->line('download_sample_import_file'); ?></button>
                        </a>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div><input class="filestyle form-control" type='file' id="my-file-selector" size='20' />
...
<script type="text/javascript">
    ...
    (function($) {
        ...
        $(document).on('click', '.select_all', function(e) {
            if (this.checked) {
                $('.table-responsive input[name="student_session_id[]"]').prop('checked', true);
            } else {
                $('.table-responsive input[name="student_session_id[]"]').prop('checked', false);
            }
        });

        $(document).on('click', '.select_all_visible', function(e) {
            if (this.checked) {
                $('input[name^="visible_student"]').prop('checked', true);
            } else {
                $('input[name^="visible_student"]').prop('checked', false);
            }
        });
...
<script>
    $(document).ready(function() {
        $(document).on('click', "#btnSubmit", function(event) {
            ...
            $.ajax({
                url: "<?php echo site_url('cbseexam/exam/importsubjectmarks') ?>",
                type: 'POST',
                dataType: 'JSON',
                data: form_data,
                contentType: false,
                ...
                success: function(data) {
                    ...
                    if (data.status == "0") {
                        ...
                        $.each(arr, function(index, value) {
                            ...
                            row.find('.studentmarks'+adm_no_csv+'_1').val(value.parameter1);
                            // Append other parameters as needed
                            row.find('.studentmarks'+adm_no_csv+'_2').val(value.parameter2);
                            row.find('.studentmarks'+adm_no_csv+'_3').val(value.parameter3);
                            row.find('.studentmarks'+adm_no_csv+'_4').val(value.parameter4);
                            row.find('.studentnote'+adm_no_csv).val(value.note);
