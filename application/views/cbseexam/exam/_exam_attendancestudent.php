...
<div class="row">
<form id="getattendance" method="post" class="" enctype="multipart/form-data">
<input  name="exam_id" type="hidden" value="<?php echo $exam_id; ?>" autocomplete="off">
    <div class="col-md-4">
        <div class="form-group">
            <label for="exampleInputEmail1">From Date</label>
            <input id="from_date" name="from_date" type="text" class="form-control date" autocomplete="off">
        </div>
    </div>
    <div class="col-md-4">
        <div class="form-group">
            <label for="exampleInputEmail1">To Date</label>
            <input id="to_date" name="to_date" type="text" class="form-control date" autocomplete="off">
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group">
...
(function ($) {
    ...
    $("#getattendance").on('submit', function (e) {
        e.preventDefault(); 
        ...
        $.ajax({
            url: "<?php echo site_url('cbseexam/exam/exam_attendance'); ?>",
            type: "POST",
            data: {
                from_date: from_date,
                to_date: to_date,
            ...
            success: function (res) {
                if (res.status === "0") {
                    ...
                    $(".table-container").html(res.page);
				    var from_date_formatted = new Date(res.from_date).toLocaleDateString('en-GB');
                    var to_date_formatted = new Date(res.to_date).toLocaleDateString('en-GB');

                    $("#from_date").val(from_date_formatted);
                    $("#to_date").val(to_date_formatted);
				}
            },
            error: function (xhr) {
                alert("<?php echo $this->lang->line('error_occurred_please_try_again'); ?>");
