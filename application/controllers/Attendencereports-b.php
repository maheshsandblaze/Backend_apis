...
class Attendencereports extends Admin_Controller
{
    ...
    public function reportbymonth()
    {
       ...
       {
           if (!$this->rbac->hasPrivilege('late_entries_report', 'can_view')) {
               access_denied();
           }
           $this->session->set_userdata('top_menu', 'Late Entries Report');
           $this->session->set_userdata('sub_menu', 'Reports/attendencereports');
           $this->session->set_userdata('subsub_menu', 'Reports/attendencereports/late_entries_report');
           $data          = array();
           $data['title'] = ' Late Entries Report';
           $class               = $this->class_model->get();
           $data['classlist']   = $class;
           $data['sch_setting'] = $this->sch_setting_detail;
           ...
           if ($this->form_validation->run() == true) {
               ...
               $data['late_entries'] = $late_entries;
           }
   
   
   
           $this->load->view('layout/header', $data);
           $this->load->view('attendencereports/late_entries_report', $data);
           $this->load->view('layout/footer', $data);
       }
    ...
    public function late_entries_analysis()
    {
        ...
        $data['total_late_students'] = $total_late_students;
        
        $this->load->view('layout/header', $data);
        $this->load->view('attendencereports/late_entries_analysis', $data);
        $this->load->view('layout/footer', $data);
    }
