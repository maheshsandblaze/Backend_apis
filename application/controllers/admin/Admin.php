...
class Admin extends Public_Controller
{
    ...
    //     $count_arr = $this->getStudentFessCount();
    ...
    //     $intakelist           = $this->schoolvacancies_model->get();
    //     // echo "<pre>";
    //     // print_r($houselist);exit;
    //     $data["admissionintakes"]   = $intakelist;
    ...
    public function dashboard()
    {
        ...
        //     ->getUnreadStaffNotification($staff_id, $role_id);
        ...
        if (strlen($end_year) == 2) {
            $end_year = substr($start_year, 0, 2) . $end_year;
        }

        /* =========================
           SYSTEM INFO
        ========================== */
        $mysqlVersion = $this->setting_model->getMysqlVersion();
        $sqlMode      = $this->setting_model->getSqlMode();
        ...
        /* =========================
           COLLECTION
        ========================== */
        $month_collection_data = $this->studentfeemaster_model
            ->getDepositAmountBetweenDate($first_day_month, $today);

        $month_collection = $this->whatever(
            $month_collection_data,
            $first_day_month,
            $today
        );
        ...
        //     $this->studentsession_model->getTotalStudentBySession()
        // )->total_student ?? 0;

        $tot_students = $this->studentsession_model->getTotalStudentBySession();

        $total_students = (!empty($tot_students) && isset($tot_students->total_student))
            ? (int) $tot_students->total_student
            : 0;


        $roles = [];
        foreach ($this->role_model->get() as $r) {
            $roles[$r['name']] = $this->role_model->count_roles($r['id']);
        }

        /* =========================
           YEARLY COLLECTION GRAPH
        ========================== */
        $academic = $this->sch_setting_detail;
        $start_month = sprintf("%02d", $academic->start_month);

        $start_date = date($start_year . '-' . $start_month . '-01');
        $end_date   = date($end_year . '-' . ($start_month - 1) . '-t');

        $deposit_data = $this->studentfeemaster_model
            ->getDepositAmountBetweenDate($start_date, $end_date);
        ...
        while ($current <= $end) {
            $months[] = $this->lang->line(strtolower(date('F', $current)));
            $month_start = date('Y-m-d', $current);
            $month_end   = date('Y-m-t', $current);

            $amount = $this->whatever($deposit_data, $month_start, $month_end);
            $yearly_collection[] = convertBaseAmountCurrencyFormat($amount ?: 0);
        ...
        /* =========================
           TODAY FEES
        ========================== */
        $today_fees = $this->studentfee_model->getTodayStudentFees();

        /* =========================
           ATTENDANCE
        ========================== */
        $student_attendance = $this->stuattendence_model
            ->getTodayDayAttendance($total_students);

        $staff_attendance = $this->Staff_model->getTodayDayAttendance();
        $total_staff = $this->Staff_model->getTotalStaff();

        $staff_attendance_percent = $total_staff > 0
            ? ($staff_attendance * 100) / $total_staff
            : 0;

        /* =========================
           ADMISSION INTAKE
        ========================== */
        $admission_intake = $this->schoolvacancies_model->get();

        // Fee summary data start
        $feesummary = array();
        $studentlist = $this->student_model->getStudents();
            ...
            ->set_output(json_encode([
                ...
                'data' => [
                    ...
                    'attendance' => [
                        ...
                        'staff_percent' => round($staff_attendance_percent, 2)
                    ],
                    'admission_intake' => $admission_intake,
                    'school_setting' => $academic
                ]
            ]));
    }
    ...
    public function sidebar_menus()
    {
        ...
        $data['menu_data']       = $this->customfield_model->get_menus();

        //print_r($data['menu_data']) ;exit;

        $this->load->view('layout/header', $data);
        $this->load->view('admin/sidebar_menus', $data);
        $this->load->view('layout/footer', $data);
    }

    public function getStudentFessCount()
    {
        $userdata = $this->customlib->getUserData();
        $staff_id = $this->customlib->getStaffID();
        $records = $this->leads_management_model->getLeadsByStaffid($staff_id);
        ...
        foreach ($records as $record) {
            ...
            $students = [];
            if (!empty($student_due_fee) && !empty($student_ids)) {
                foreach ($student_due_fee as $student_due_fee_key => $student_due_fee_value) {
                    ...
                    if (!empty($a)) {
                        ...
                        if ($amt_due <= ($amount + $amount_discount)) {
                            $paid_students += 1;
                            if ($feetype_id == 2 || $feetype_id == '2') {
                                $first_term_leads += 1;
                            } else if ($feetype_id == 3 || $feetype_id == '3') {
                                $second_term_leads += 1;
                            } else if ($feetype_id == 4 || $feetype_id == '4') {
                                $third_term_leads += 1;
                            }
            ...
            foreach ($students as $student) {
                ...
                } else if ($balance == 0.00 || $balance == '0.00') {
                    $paid_students += 1;
                }
            }
        }
        ...
        return  [
            ...
            'paid' =>  $paid_students,
            'first_term' => $first_term_leads,
            'second_term' => $second_term_leads,
            'third_term' => $third_term_leads,
            'arr' => $arr
        ];
    }
    ...
    ========================== */
        ...
        if ($session_array) {
            ...
            $current_session = $this->session->userdata('session_array');
        } else {
        ...
        // echo "<pre>";print_r($current_session);exit;
    ...
    ========================== */
        ...
        $session_id =  $input['popup_session'];
    ...
    ========================== */
        return $this->output
            ...
            ->set_output(json_encode([
                'status' => true,
                'message' => $this->lang->line('session_changed_successfully'),
    ...
    public function search()
    {
        ...
        $search_text = isset($input['search_text']) ? trim($input['search_text']) : '';
    ...
    /*public function gallery() {
		...
		if ($this->input->post('category_name')) {
			...
			if (!empty($category_name)) {
				//echo "test";exit;
				$data_insert = ['category_name' => $category_name];
				$this->admin_model->add_category($data_insert);
	
				ob_clean(); 
				redirect('admin/admin/gallery', 'refresh');
				exit;
			}
		}
	
		$data['g_categories'] = $this->admin_model->get_categories();
    ...
    public function attendance_analysis()
    {
        ...
        $this->load->view('attendencereports/attendanceanalysis', $data);
