...
class Route_model extends MY_Model
{
    ...
    {
        $query = $this->db->select('transport_route.route_title, vehicles.vehicle_no, vehicles.vehicle_model, vehicles.driver_name, vehicles.driver_contact')
            ->join('transport_route', 'transport_route.id = vehicle_routes.route_id')
            ->join("vehicles", "vehicle_routes.vehicle_id = vehicles.id")
    ...
    public function check_duplicate($route_title)
    {
        return $this->db->where('route_title', $route_title)
            ->get('transport_route')
            ->row();
    }
}
