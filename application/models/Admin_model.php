...
class Admin_model extends CI_Model
{
    ...
    public $client_service               = "newlayout";
    ...
    public function validate_token()
    {
        ...
        if (!$authHeader) {
            return json_output(401, ['status' => 0, 'message' => 'Token missing']);
        }
        ...
        $query = $this->db->get('admin_authentication');
    
        if ($query->num_rows() === 0) {
            return json_output(401, ['status' => 0, 'message' => 'Invalid or expired token']);
        }
    
        return $query->row(); // authenticated user
    }
	...
	public function delete_g_category($id)
	{
		$this->db->where('id', $id);
