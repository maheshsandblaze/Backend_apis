...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null, $route_pickup_point_id = null)
    {
        ...
        if ($route_pickup_point_id != null) {
            $sql .= " AND `student_session`.`route_pickup_point_id` =" . $this->db->escape($route_pickup_point_id);
        }
    ...
    // getStudentAcadamicFees
    public function getStudentAcadamicFees($student_session_id, $from_date = null, $to_date = null, $acadamictype)
    {
        // Sanitize input
        ...
        $sql = "
            ...
            INNER JOIN 
                fee_session_groups fsg 
                ON sfm.fee_session_group_id = fsg.id
            INNER JOIN 
                fee_groups fg 
                ON fg.id = fsg.fee_groups_id
            WHERE 
                sfm.student_session_id = $student_session_id";
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                if ($acadamictype == 1) {
                    ...
                    $res     = $this->getDueFeeByAcadamicFeeSessionGroup($fee_session_group_id, $student_fees_master_id, $from_date, $to_date);

                    if (!empty($res)) {
                        $result_value->fees     = $res;
                    }
                } else if ($acadamictype == 2) {

                    $res = $this->getDueFeeByNonAcadamicFeeSessionGroup($fee_session_group_id, $student_fees_master_id, $from_date, $to_date);
                    ...
                    $res = $this->getDueFeeByFeeSessionGroup($fee_session_group_id, $student_fees_master_id, $from_date, $to_date);
    ...
    public function getDueFeeByAcadamicFeeSessionGroup($fee_session_groups_id, $student_fees_master_id, $from_date = null, $to_date = null)
    {

        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.due_date,fee_groups_feetype.fine_amount,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id = student_fees_master.fee_session_group_id INNER JOIN fee_groups_feetype on  fee_groups_feetype.fee_session_group_id = fee_session_groups.id  INNER JOIN fee_groups on fee_groups.id=fee_groups_feetype.fee_groups_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " AND 
        feetype.type NOT IN ('Belt', 'Book Fee', 'Uniform Fee 1', 'Uniform Fee 2','Uniform Fee','Sports Uniform Boys Fee','Sports Uniform Girls Fee') ";
        ...
        //     $sql .= " AND DATE(student_fees_deposite.created_at) BETWEEN '$from_date' AND '$to_date'";
        // } elseif ($from_date) {
        //     $from_date = $this->db->escape_str($from_date);
        //     $sql .= " AND DATE(student_fees_deposite.created_at) >= '$from_date'";
        // } elseif ($to_date) {
        //     $to_date = $this->db->escape_str($to_date);
        //     $sql .= " AND DATE(student_fees_deposite.created_at) <= '$to_date'";
        // }
    ...
    public function getDueFeeByNonAcadamicFeeSessionGroup($fee_session_groups_id, $student_fees_master_id, $from_date = null, $to_date = null)
    {

        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.due_date,fee_groups_feetype.fine_amount,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id = student_fees_master.fee_session_group_id INNER JOIN fee_groups_feetype on  fee_groups_feetype.fee_session_group_id = fee_session_groups.id  INNER JOIN fee_groups on fee_groups.id=fee_groups_feetype.fee_groups_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " AND 
        feetype.type IN ('Belt', 'Book Fee', 'Uniform Fee 1', 'Uniform Fee 2','Uniform Fee','Sports Uniform Boys Fee','Sports Uniform Girls Fee') ";
            ...
            foreach ($result_value as $key => $value) {
                ...
                if (!empty($return)) {
                    foreach ($return as $r_key => $r_value) {
                        ...
                        $a['payment_mode']           = $r_value->payment_mode;
                        $a['inv_no']                 = $r_value->inv_no;
                        $a['collected_by'] =    $r_value->received_by;
    ...
    public function getFeeDisountBetweenDate($start_date, $end_date)
    {
    ...
    public function getTodayCollectedfees($current_date)
    {
        $sql = "SELECT sum(amount) as today_amount from student_fees_print where DATE(created_at) = ?";

        $query  = $this->db->query($sql, array($current_date));
        return $query->row()->today_amount;
    }

    // get datawise feecollection report 


    public function getFeeCollectionReportByDate($start_date, $end_date, $feetype_id = null, $received_by = null, $group = null, $class_id = null, $section_id = null)
    {
        $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,feetype.is_system,student_fees_master.student_session_id,students.admission_no')->from('student_fees_deposite');
        $this->db->join('fee_groups_feetype', 'fee_groups_feetype.id = student_fees_deposite.fee_groups_feetype_id');
        $this->db->join('fee_groups', 'fee_groups.id = fee_groups_feetype.fee_groups_id');
    ...
    public function getfeedetails($feetypes, $student_session_id)
    {
        $this->db->select('student_fees_master.id as fee_master_id, fee_groups_feetype.id as fee_group_feetype_id, fee_groups_feetype.amount as fee_amount,feetype.type as feestype');
        ...
        $this->db->where('feetype.type', $feetypes);
    ...
    //Student Due Dates Fetch
    public function getstudentduedates($student_session_id)
    {
        $this->db->select('
            student_fees_master.id as fee_master_id, 
            fee_groups_feetype.id as fee_group_feetype_id, 
            fee_groups_feetype.amount as fee_amount, 
            fee_groups_feetype.due_date
        ');
        $this->db->from('student_fees_master'); // Start from student_fees_master
        $this->db->join('fee_session_groups', 'fee_session_groups.id = student_fees_master.fee_session_group_id');
        $this->db->join('fee_groups_feetype', 'fee_groups_feetype.fee_session_group_id = fee_session_groups.id'); // Correct join condition
        $this->db->where('student_fees_master.student_session_id', $student_session_id);
        $this->db->group_by('fee_groups_feetype.due_date');
    ...
    // Student Fees Fetch based on the due dates
    public function getStudentFeesbyduedate($student_session_id, $due_date)
    {
        if (!strtotime($due_date)) {
            return false; // Return false if the date is invalid
        }

        $due_date = date('Y-m-d', strtotime($due_date)); // Convert to proper date format

        $this->db->select('
            student_fees_master.id as fee_master_id, 
            fee_groups_feetype.id as fee_group_feetype_id, 
            fee_groups_feetype.amount as fee_amount,
            fee_groups_feetype.due_date as due_date,
            feetype.type as feestype,
            feetype.code as feecodes,
        ...
        $this->db->where('student_fees_master.student_session_id', $student_session_id);
        $this->db->where('fee_groups_feetype.due_date', $due_date);
    ...
    public function fee_discount_payments($data)
    {
        if (!empty($data)) {
            $collected_fees = array();
            ...
            foreach ($data as $d_key => $d_value) {
                ...
                if ($d_value['fee_category'] == "discount") {
    ...
    // getstudentassigndiscount

    public function getstudentassigndiscount($feemaster_id, $fee_groups_feetype_id)
    {



        $this->db->where('student_fees_master_id', $feemaster_id);
        $this->db->where('fee_groups_feetype_id', $fee_groups_feetype_id);
        //  $this->db->where('id', $data['student_fees_deposite_id']);

        $q = $this->db->get('student_discount_approve');
        if ($q->num_rows() > 0) {
            //  $desc = $data['amount_detail']['description'];
            $this->db->trans_start();
            $row = $q->row();
    ...
    public function fee_discount_approve($data, $send_to, $student_fees_discount_id = null)
    {
        if ($data['fee_category'] == "fees") {
            # code...
            $this->db->where('student_fees_master_id', $data['student_fees_master_id']);
        ...
        if ($q->num_rows() > 0) {
            ...
            $a[$inv_no]                      = $data['amount_detail'];
            $data['amount_detail']           = json_encode($a);
            $this->db->update('student_discount_approve', $data);

            if ($student_fees_discount_id != null) {
                $this->db->where('id', $student_fees_discount_id);
                $this->db->update('student_discount_approve', array('status' => 'applied', 'description' => $desc, 'payment_id' => $row->id . "//" . $inv_no));
            ...
            $desc                            = $data['amount_detail']['description'];
            $data['amount_detail']           = json_encode(array('1' => $data['amount_detail']));
            $this->db->insert('student_discount_approve', $data);
            $inserted_id = $this->db->insert_id();
            if ($student_fees_discount_id != null) {
                $this->db->where('id', $student_fees_discount_id);
                $this->db->update('student_discount_approve', array('status' => 'applied', 'description' => $desc, 'payment_id' => $inserted_id . "//" . "1"));
            }
    ...
    public function getStudentDiscounts($student_session_id)
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id   WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                $fee_session_group_id   = $result_value->fee_session_group_id;
                $student_fees_master_id = $result_value->id;
                $result_value->fees     = $this->getstudentdiscountsgroup($fee_session_group_id, $student_fees_master_id);
    ...
    public function getstudentdiscountsgroup($fee_session_groups_id, $student_fees_master_id)
    {

        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.due_date,fee_groups_feetype.fine_amount,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type, IFNULL(student_discount_approve.id,0) as `student_fees_deposite_id`,IFNULL(student_discount_approve.id,0) as discount_approve_id,IFNULL(student_discount_approve.is_active,'') as discount_status, IFNULL(student_discount_approve.amount_detail,0) as `amount_detail` FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id = student_fees_master.fee_session_group_id INNER JOIN fee_groups_feetype on  fee_groups_feetype.fee_session_group_id = fee_session_groups.id  INNER JOIN fee_groups on fee_groups.id=fee_groups_feetype.fee_groups_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id INNER JOIN student_discount_approve on student_discount_approve.student_fees_master_id=student_fees_master.id and student_discount_approve.fee_groups_feetype_id=fee_groups_feetype.id WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " order by fee_groups_feetype.due_date ASC";
    ...
    public function updatediscount($data)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $id = $row->id;
            $this->db->where('id', $id);
            $this->db->update('student_discount_approve', ['is_active' => 'yes']);
