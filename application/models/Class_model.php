...
class Class_model extends MY_Model
{
    ...
    {
        parent::__construct();
        if ($this->db->table_exists('leads_management')) {
    ...
    public function get($id = null, $classteacher = null)
    {
        ...
        } else if (isset($role_id) && $role_id == 8 && $staff_id && $this->db->table_exists('leads_management')) {
            $class_ids = $this->leads_management_model->getClassByStaffId($staff_id);
            if (!empty($class_ids)) {
                $this->db->select()->from('classes');
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $record_id = $data['id'];
            $message   = UPDATE_RECORD_CONSTANT . " On classes id " . $record_id;
            $action    = "Update";
        } else {
            $this->db->insert('classes', $data);
            $record_id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On classes id " . $record_id;
            $action    = "Insert";
        }
    ...
    public function getClassIdByName($class)
    {
