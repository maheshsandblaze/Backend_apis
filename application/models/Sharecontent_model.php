...
class Sharecontent_model extends MY_Model
{
    ...
    public function getStudentsharelist_mobile($student_id, $class_id, $section_id)
    {
        $sql = "SELECT `share_contents`.*, `staff`.`name`, `staff`.`surname`, `staff`.`employee_id`, staff_roles.role_id 
                FROM `share_contents` 
        ...
        return $this->db->query($sql, [$student_id, $class_id, $section_id])->result_array();
    }
    
    public function getParentsharelist_mobile($user_parent_id, $class_id, $section_id)
    {
        $sql = "SELECT `share_contents`.*, `staff`.`name`, `staff`.`surname`, `staff`.`employee_id`, staff_roles.role_id 
                FROM `share_contents` 
                JOIN `staff` ON `share_contents`.`created_by` = `staff`.`id` 
        ...
        return $this->db->query($sql, [$user_parent_id, $class_id, $section_id])->result_array();
    }
