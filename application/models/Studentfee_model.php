...
class Studentfee_model extends MY_Model
{
    ...
    public function remove($id, $sub_invoice)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if (!empty($a)) {
                ...
                $deleted_record = (array) $result;
                $this->db->insert('student_fees_deposite_deleted', $deleted_record);
    ...
    public function discountremove($id, $sub_invoice)
    {
        $this->db->trans_start();
        $this->db->trans_strict(false);
        ...
        if ($q->num_rows() > 0) {
            ...
            if (!empty($a)) {
                ...
                $message = UPDATE_RECORD_CONSTANT . " On student fees discount id " . $id;
                $action = "Update";
                $record_id = $id;
                $this->log($message, $record_id, $action);
            } else {
                ...
                $this->db->where('id', $id);
                $this->db->delete('student_discount_approve');
                $message = DELETE_RECORD_CONSTANT . " On student fees discount id " . $id;
                $action = "Delete";
                $record_id = $id;
                $this->log($message, $record_id, $action);
            }
