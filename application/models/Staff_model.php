...
class Staff_model extends MY_Model
{
        ...
        if ($this->session->has_userdata('admin')) {
            if ($this->staffrole->id != 7) {
                if ($this->superadmin_visible == 'disabled') {
                    ...
                    // $this->db->where("roles.id !=", 7);
                    $this->db->where('staff.is_invisibles_user !=', 1);
                }
            }
        }
    ...
    public function get_agents($id = null)
    {
        $this->db->select('staff.*,languages.language,languages.is_rtl,roles.name as user_type,roles.id as role_id')->from('staff')->join("staff_roles", "staff_roles.staff_id = staff.id", "left")->join("roles", "staff_roles.role_id = roles.id", "left")->join("languages", "languages.id = staff.lang_id", "left");
        $this->db->where('roles.id', 8);

        if ($this->session->has_userdata('admin')) {
            if ($this->staffrole->id != 7) {
                if ($this->superadmin_visible == 'disabled') {
                    $this->db->where("roles.id !=", 7);
                    $this->db->where('staff.is_invisibles_user !=',);
                } else if ($this->staffrole->id != 7) {
                    // $this->db->where("roles.id !=", 7);
                    $this->db->where('staff.is_invisibles_user !=', 1);
                }
            }
    ...
    public function getStaffRole($id = null)
    {
        ...
        $this->db->join('staff_roles', 'staff_roles.role_id = roles.id');
        $this->db->join('staff', 'staff_roles.staff_id = staff.id');
        ...
        $this->db->where("roles.is_active", "yes");
    ...
    public function searchFullText($searchterm, $active)
    {
        ...
        if ($this->staffrole->id != 7) {
            if ($this->superadmin_visible == 'disabled') {
                $condition = "and roles.id = 7 and staff.is_invisibles_user = 1 ";
            } else {
                $condition = " and staff.is_invisibles_user != 1 ";
            }
        }
    ...
    public function staff_report($condition)
    {
        ...
        // echo $this->superadmin_visible;exit;

        if ($this->session->has_userdata('admin')) {
            if ($this->staffrole->id != 7) {
                if ($this->superadmin_visible == 'disabled') {
                    $rolescondition = " and roles.id != 7 and staff.is_invisibles_user != 1";
                } else {
                    $rolescondition = " and staff.is_invisibles_user != 1 ";
                }
            }
        }
    ...
    public function insert_recruitment($data)
    {
        //echo "model"; print_r($data);exit;
        $this->db->insert('staff_recruitment', $data);
        return true;
    }

    public function getstaffrecruitment()
    {

        $query = $this->db->get('staff_recruitment');
        return $query->result_array();
    }

    public function delete_recruitment($id)
    {
        $this->db->delete('staff_recruitment', array('id' => $id));
    }
}
