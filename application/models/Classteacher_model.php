...
class Classteacher_model extends MY_Model
{
    ...
    public function addSubjectTeacher($data)
    {
        ...
        //=======================Code Start===========================
        if (isset($data["id"])) {
            $this->db->where("id", $data["id"])->update("subject_teacher", $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  class teacher id " . $data["id"];
            $action    = "Update";
            $record_id = $data["id"];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert("subject_teacher", $data);
            $id        = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On class teacher id " . $id;
            $action    = "Insert";
            $record_id = $id;
    ...
    public function getSubjectTeachersList()
    {
        $query = $this->db->select('staff.*,subject_teacher.id as stid,subject_teacher.class_id,subject_teacher.section_id,classes.class,sections.section,subjects.name as subject_name')->join("staff", "subject_teacher.staff_id = staff.id")->join("classes", "subject_teacher.class_id = classes.id")->join("sections", "subject_teacher.section_id = sections.id")->join("subjects", "subject_teacher.subject_id = subjects.id")->where("staff.is_active", 1)->where("subject_teacher.session_id", $this->current_session)->get("subject_teacher");
        return $query->result_array();
    }
    ...
    public function getSubjectsByStaff(){
        
        $userdata = $this->customlib->getUserData();
        $staff_id = $userdata["id"];
        
        $this->db->select('subject_teacher.*');
        $this->db->from('subject_teacher');
        $this->db->where('staff_id', $staff_id);    
        $this->db->where('session_id', $this->current_session);       
        $exams=$this->db->get();
        $result= $exams->result_array();
        return $result;
    }
