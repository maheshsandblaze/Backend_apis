...
class Sendwhatsapp extends Public_Controller
{
    ...
    //     }
    //     $this->session->set_userdata('top_menu', 'Communicate');
    //     $this->session->set_userdata('sub_menu', 'sendwhatsapp/compose_whatsapp');
    //     $data['title']             = 'Add Mailsms';
    ...
    //     $this->load->view('layout/header');
    //     $this->load->view('admin/sendwhatsapp/compose_sms', $data);
    //     $this->load->view('layout/footer');
    // }
...
//         $this->form_validation->set_rules('message_type_group', 'Message Type', 'required');
...
//             $message_type  = $this->input->post('message_type_group');
//             $video         = $this->input->post('video_link_grp');
//             $data          = array(
//                 'is_group'        => 1,
//                 'title'           => $message_title,
//                 'message'         => $message,
...
//                                 $this->smsgateway->sendWhatsapp($user_mail_value['mobileno'], $message, $message_type, $video);
//                             }
...
//                 'message_type_group' => form_error('message_type_group')
//             );
//             if ($send_type == 'schedule') {
...
//     }
    ...
    public function send_group_sms()
    {
        ...
        if (isset($input['send_type']) && $input['send_type'] === 'schedule') {
        ...
        if (
            ...
            echo json_encode([
                'status' => 1,
                'msg' => [
                    'user' => 'The Recipients field is required.'
                ]
            ]);
            return;
        }
        ...
        if (!$this->form_validation->run()) {
            ...
            echo json_encode(['status' => 1, 'msg' => $errors]);
            return;
        }
    
        // Collect input
        $send_type     = $input['send_type'];
        $sms_mail      = $input['group_send_by'] ?? [];
        $message       = $input['group_message'];
        $message_title = $input['group_title'] ?? '';
        $message_type  = $input['message_type_group'];
        $video         = $input['video_link_grp'] ?? '';
        $template_id   = $input['group_template_id'] ?? null;
    
        $user_array = [];
    
        // Build user list
        foreach ($input['user'] as $users_value) {
            if ($users_value === "student") {
                $student_array = $this->student_model->get();
        ...
        $data = [
            ...
            'send_sms'        => 1,
            'send_through'    => json_encode($sms_mail),
            'sms_template_id' => $input['template_id'] ?? null,
        ...
        if ($send_type === 'schedule') {
            ...
            $data['schedule_date_time'] = date(
                'Y-m-d H:i:s',
                $this->customlib->dateTimeformatTwentyfourhour($input['schedule_date_time'], false)
            );
        }
...
//         $this->form_validation->set_rules('message_type_individual', 'Message Type', 'required');
...
//             $message_type  = $this->input->post('message_type_individual');
//             $video         = $this->input->post('video_link_indv');
//             $data          = array(
//                 'is_individual'   => 1,
//                 'title'           => $message_title,
//                 'message'         => $message,
...
//                 'message_type_individual'   => form_error('message_type_individual')
//             );
//             if ($send_type == 'schedule') {
//                 $data['schedule_date_time'] = form_error('schedule_date_time');
//             }
...
//     }
    ...
    public function send_individual_sms()
    {
        ...
        $this->form_validation->set_rules('message_type_individual', 'Message Type', 'required');
        $this->form_validation->set_rules('individual_send_type', 'Send Type', 'required');
    
        if (isset($input['individual_send_type']) && $input['individual_send_type'] === 'schedule') {
            $this->form_validation->set_rules('schedule_date_time', 'Schedule Date Time', 'required');
        }
    
        // Validate user_list array
        if (
            !isset($input['user_list']) ||
            empty($input['user_list'])
        ) {
            echo json_encode([
                'status' => 1,
                'msg' => [
                    'user_list' => 'The Recipient field is required.'
                ]
            ]);
            return;
        }
        ...
        if (!$this->form_validation->run()) {
            $errors = [
                ...
                'message_type_individual' => form_error('message_type_individual'),
                'individual_send_by'      => form_error('individual_send_by'),
                'individual_send_type'    => form_error('individual_send_type'),
            ];
            if ($input['individual_send_type'] === 'schedule') {
                $errors['schedule_date_time'] = form_error('schedule_date_time');
            }
        ...
        $sms_mail      = $input['individual_send_by'] ?? [];
        $message       = $input['individual_message'];
        $message_title = $input['individual_title'] ?? '';
        $message_type  = $input['message_type_individual'];
        $video         = $input['video_link_indv'] ?? '';
        $template_id   = $input['individual_template_id'] ?? null;
    
        // Build user array
        $userlisting = json_decode($input['user_list']);
        $user_array  = [];
        ...
        if ($send_type === 'send_now') {
            ...
            $responseMsg = 'Message scheduled successfully';
        }
...
//         $this->form_validation->set_rules('user[]', $this->lang->line('recipient'), 'required');
//         $this->form_validation->set_rules('message_type_class', 'Message Type', 'required');
...
//             $message_type  = $this->input->post('message_type_class');
//             $video         = $this->input->post('video_link_cls');
...
//                 'user[]'                => form_error('user[]'),
//                 'message_type_class'    => form_error('message_type_class')
//             );
//             if ($send_type == 'schedule') {
//                 $data['schedule_date_time'] = form_error('schedule_date_time');
//             }
...
//     }
    ...
    public function send_class_sms()
    {
        ...
        $this->form_validation->set_rules('message_type_class', 'Message Type', 'required');
        $this->form_validation->set_rules('class_send_type', 'Send Type', 'required');
    
        if (isset($input['class_send_type']) && $input['class_send_type'] === 'schedule') {
            $this->form_validation->set_rules('schedule_date_time', 'Schedule Date Time', 'required');
        }
        ...
        if (!$this->form_validation->run()) {
            $errors = [
                ...
                'user'               => form_error('user'),
                'message_type_class' => form_error('message_type_class'),
                'class_send_type'    => form_error('class_send_type'),
            ];
            if ($input['class_send_type'] === 'schedule') {
                $errors['schedule_date_time'] = form_error('schedule_date_time');
            }
        ...
        // Collect input
        $send_type     = $input['class_send_type'];
        $sms_mail      = $input['class_send_by'] ?? [];
        $message       = $input['class_message'];
        $message_title = $input['class_title'] ?? '';
        $sections      = $input['user'];
        $class_id      = $input['class_id'];
        $message_type  = $input['message_type_class'];
        $video         = $input['video_link_cls'] ?? '';
        $template_id   = $input['class_template_id'] ?? null;
    
        $user_array = [];
    
        // Build user list by class/section
        foreach ($sections as $section_value) {
            $userlisting = $this->student_model->searchByClassSection($class_id, $section_value);
            if (!empty($userlisting)) {
                foreach ($userlisting as $student) {
                    $user_array[] = [
                        'user_id'  => $student['id'],
                        'email'    => $student['email'],
                        'mobileno' => $student['mobileno'],
    ...
    public function test_sms()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->smsgateway->sendSMS($this->input->post('mobile'), ('Pragati Vidyaniketan High School SMS Test Successful.'));
    ...
    public function test_whatsapp()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->smsgateway->sendWhatsapp($this->input->post('mobile'), ('Pragati Vidyaniketan High School Whatsapp Message Test Successful.'));
