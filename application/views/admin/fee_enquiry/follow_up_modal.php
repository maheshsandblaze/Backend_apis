...
?>
        ...
        <div class="ptbnull">
            ...
            <h4 class="box-title titlefix pb5"><?php echo $this->lang->line('follow_up'); ?> (<?php echo $student_data['firstname'] . ' ' . $student_data['lastname'];?>)</h4>
...
?>
            ...
            <div class="task-info task-single-inline-wrap ptt10">
                <h5><span class="text-dark">Father Name:</span> <?php echo $student_data['father_name']; ?></h5>
                <h5><span class="text-dark">Father Phone:</span> <?php echo $student_data['father_phone']; ?></h5>
                <h5><span class="text-dark">Mother Name:</span> <?php echo $student_data['mother_name']; ?></h5>
                <h5><span class="text-dark">Mother Phone:</span> <?php echo $student_data['mother_phone']; ?></h5>
                <h5><span class="text-dark">Current Address:</span> <?php echo $student_data['current_address']; ?></h5>
                <h5><span class="text-dark">Permanent Address:</span> <?php echo $student_data['permanent_address']; ?></h5>
                <h5><span class="text-dark"><?php echo $this->lang->line('email'); ?>:</span> <?php echo $student_data['email']; ?></h5>    
            </div>
        </div>
    </div>
</div>
<script>
    ...
    $("#folow_up_data").on('submit', (function (e) {
        ...
        $.ajax({
            url: "<?php echo site_url("admin/enquiry/fee_follow_up_insert/") ?>",
            type: "POST",
    ...
    function follow_up_new(id, status, created_by) {
        console.log(id, status, created_by);
        $.ajax({
            url: '<?php echo base_url(); ?>admin/enquiry/feefollow_up/' + id + '/' + status+ '/' + created_by,
            success: function (data) {
                $('#getdetails_follow_up').html(data);
                $.ajax({
                    url: '<?php echo base_url(); ?>admin/enquiry/fee_followup_list/' + id,
                    success: function (data) {
                        $('#timeline').html(data);
                    },
                    error: function () {
    ...
    function changeStatus(status, id, created_by) {
        $.ajax({
            url: '<?php echo base_url(); ?>admin/enquiry/fee_change_status/',
            type: 'POST',
            dataType: 'json',
