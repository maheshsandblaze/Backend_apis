...
class Cbseexam_result_model extends MY_Model
{
    ...
    */
    ...
    public function get_cbse_template($cbse_template_id)
    {
    ...
    */
    ...
    public function getmultiexamwithouttermresult($student_session_id, $cbse_template_id, $exam_id, $cbse_exam_student_id)
    {
        $this->db->select('cbse_exam_students.id as cbse_exam_student_id, cbse_template_term_exams.weightage, cbse_template_term_exams.cbse_exam_id,SUM(cbse_student_subject_marks.marks) as student_marks, cbse_student_subject_marks.is_absent');
        $this->db->from('cbse_template_term_exams');
        $this->db->join('cbse_exam_students', 'cbse_exam_students.cbse_exam_id=cbse_template_term_exams.cbse_exam_id');
        $this->db->join('cbse_exams', 'cbse_exams.id=cbse_template_term_exams.cbse_exam_id');
        $this->db->join('cbse_student_subject_marks', 'cbse_student_subject_marks.cbse_exam_student_id=cbse_exam_students.id');
        $this->db->where('cbse_exam_students.student_session_id', $student_session_id);
        $this->db->where('cbse_template_term_exams.cbse_template_id', $cbse_template_id);
        $this->db->where('cbse_exams.id !=', $exam_id);
        if (is_array($cbse_exam_student_id) && !empty($cbse_exam_student_id)) {
            $this->db->where_in('cbse_student_subject_marks.cbse_exam_student_id', $cbse_exam_student_id);
        } elseif (!is_array($cbse_exam_student_id)) {
            $this->db->where('cbse_student_subject_marks.cbse_exam_student_id', $cbse_exam_student_id);
        }
